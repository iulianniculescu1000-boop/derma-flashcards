...
for course, items in by_course.items():
    if not any(it['needs_label'] for it in items):
        continue
    ...
    for p in pages_needed:
        out["pages"][str(p)] = doc[p - 1].get_text("text")
    ...
    for it in sorted(items, key=lambda x: (x['page'], x['file'])):
        out["images"].append({
            ...
            "needs_label": it["needs_label"],
        })

    fname = os.path.join(OUT_DIR, course.replace(" ", "_") + ".json")
    with open(fname, 'w', encoding='utf-8') as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    n_new = sum(1 for it in items if it['needs_label'])
    print(f"{fname}  ({n_new} need labels)")
